...
function resetDetailEditor() {
  ...
  if (addDetailToDraft) addDetailToDraft.textContent = "＋ Añadir detalle";
    ...
    <article class="detected-detail-card">
      ...
      <div class="detected-detail-info">
        ...
        <div class="detected-detail-meta">
          ...
          <span class="detail-time">▶ ${formatTimestamp(item.timestampStart)}</span>
        </div>
      </div>
      <div class="detected-detail-actions">
        <button type="button" data-edit-detail="${index}" aria-label="Editar">✏️</button>
        <button type="button" data-delete-detail="${index}" aria-label="Eliminar">🗑️</button>
      </div>
    </article>
  `).join("");
...
function editDraftDetail(index) {
  ...
  addDetailToDraft.textContent = "✓ Guardar cambios";
...
async function createSupabaseVideo({ title, description, url }) {
  ...
  const sourceType = url.includes("instagram.com") ? "Instagram" : url.includes("tiktok.com") ? "TikTok" : (url.includes("youtube.com") || url.includes("youtu.be")) ? "YouTube" : "Vídeo";
...
discoveryForm.addEventListener("submit", async event => {
  ...
  try {
    ...
    renderMarkers();
    closeAddDiscovery();
    showToast(`✓ ${discoveryDraft.length} detalles guardados para todos`);
